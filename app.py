...
with sim_col1:
    ...
    st.write("📁 Data location: `Hospital_local_machine/Training/` (never uploaded anywhere)")
...
with sim_col2:
    if run_button:
        ...
        status_text.success(f"✅ {selected_hospital}'s update received and verified by central platform.")

        # Show a plausible local accuracy jump using real saved numbers as reference
        ...
        c3.metric("Data shared with platform", "0 images", "Only weights")
    else:
        st.info("👆 Select a hospital and click **Run Local Training Round** to see the local training + encrypted upload process step by step.")

# --- Real inference on any uploaded image, tied to the selected hospital ---
st.markdown("##### 🔬 Test this hospital's model on any MRI scan")
st.caption(f"Upload any brain MRI image to see how the model classifies it — proves the trained model generalizes beyond a fixed demo set, not just canned examples.")

uploaded_sim_image = st.file_uploader(
    "Upload an MRI scan (jpg/png)",
    type=["jpg", "jpeg", "png"],
    key="sim_image_upload"
)
...
if uploaded_sim_image is not None:
    _model = load_inference_model()
    if _model is None:
        st.warning("Model file (`medfed_model.pt`) not found alongside this app — add it to enable live inference here.")
    else:
        img = Image.open(uploaded_sim_image).convert('RGB')
        img_col, pred_col = st.columns([1, 1])

        with img_col:
            st.image(img, caption=f"Uploaded scan — tested against {selected_hospital}'s model", width=280)
        ...
        with pred_col:
            ...
            pred_classes = ['glioma', 'meningioma', 'notumor', 'pituitary']
            results = {pred_classes[i]: float(probs[i]) for i in range(len(pred_classes))}
            sorted_results = dict(sorted(results.items(), key=lambda x: x[1], reverse=True))
            top_class, top_conf = list(sorted_results.items())[0]
...
# --- Real inference on any uploaded image, tied to the selected hospital ---
st.markdown("##### 🔬 Test this hospital's model on any MRI scan")
st.caption("Upload any brain MRI image to see how the model classifies it — proves the trained model generalizes beyond a fixed demo set.")

uploaded_sim_image = st.file_uploader("Upload an MRI scan (jpg/png)", type=["jpg", "jpeg", "png"], key="sim_image_upload")
...
if uploaded_sim_image is not None:
    _model = load_inference_model()
    if _model is None:
        st.warning("Model file not found alongside this app.")
    else:
        img = Image.open(uploaded_sim_image).convert('RGB')
        img_col, pred_col = st.columns([1, 1])
        with img_col:
